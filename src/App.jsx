...
export default function App() {
  ...
  // ИСПРАВЛЕННЫЙ ТЕМАТИЗАТОР (ФИКС КРИСТАЛЬНОГО ГЛАЗА)
  ...
  const filteredData = useMemo(() => {
    ...
    Object.entries(spellsData).forEach(([letter, spells]) => {
      const filtered = spells.filter(s => {
        ...
        // ЛОГИКА ДИСТАНЦИИ (ЧИСЛОВАЯ ПРОВЕРКА)
        let matchesDist = true;
        if (distFilter !== "all") {
          const sRange = parseInt(s.range) || 0;
          if (distFilter === "self") matchesDist = s.range?.toLowerCase().includes("на себя");
          else if (distFilter === "touch") matchesDist = s.range?.toLowerCase().includes("касание");
          else if (distFilter === "30") matchesDist = sRange > 0 && sRange <= 30;
          else if (distFilter === "60") matchesDist = sRange > 0 && sRange <= 60;
          else if (distFilter === "90") matchesDist = sRange > 0 && sRange <= 90;
          else if (distFilter === "120") matchesDist = sRange > 0 && sRange <= 120;
        }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-amber-50 font-serif overflow-x-hidden">
      <header className="p-10 pb-6 max-w-[1600px] mx-auto">
        <h1 className="text-4xl md:text-6xl font-black text-amber-500 uppercase italic leading-tight tracking-tighter text-shadow-lg">АРХИВ ПАРОМЕХАНИКА</h1>
        <p className="text-amber-700 text-sm mt-2 uppercase tracking-widest font-sans font-bold">v.6.7 | Система точного поиска</p>
      </header>
      ...
      <div className="flex gap-6 px-4 md:px-10 pb-10 max-w-[1600px] mx-auto relative">
        ...
        <div className="w-24 md:w-36 flex-shrink-0 flex flex-col items-center sticky top-6 h-[calc(100vh-48px)] z-[150]">
          <div className="w-full bg-amber-600 pt-5 pb-3 rounded-t-2xl border-b-2 border-black/20 flex flex-col items-center shadow-lg">
             <span className="text-[10px] md:text-xs font-black text-black text-center uppercase">Система</span>
          </div>
              ...
              <div className="sticky top-0 z-[110] bg-[#0a0a0a] pt-2 pb-4 border-b-2 border-amber-500/50">
                <div className="flex flex-col gap-3 bg-[#111] p-4 border border-amber-900/30 rounded-lg shadow-2xl">
                  ...
                  <div className="flex flex-wrap gap-3 items-center">
                    <input type="text" placeholder="Поиск по названию..." className="bg-black border border-amber-700/50 p-3 rounded text-amber-100 flex-grow outline-none focus:border-amber-400 text-sm font-sans" value={search} onChange={(e) => setSearch(e.target.value)} />
                    
                    {/* ТУМБЛЕРЫ КОМПОНЕНТОВ */}
                    <div className="flex bg-black border border-amber-900/50 rounded p-1 gap-1">
                      {['V', 'S', 'M'].map(c => (
                        <button key={c} onClick={() => setCompFilter(prev => prev.includes(c) ? prev.filter(x => x !== c) : [...prev, c])} className={`w-8 h-8 rounded text-[10px] font-black transition-all ${compFilter.includes(c) ? 'bg-amber-600 text-black shadow-inner' : 'text-amber-900 hover:text-amber-600'}`}>
                          {c}
                        </button>
                      ))}
                    </div>
                    ...
                    <select className="bg-black border border-amber-700/50 p-3 rounded text-amber-100 text-xs outline-none font-bold uppercase" value={distFilter} onChange={(e) => setDistFilter(e.target.value)}>
                      <option value="all">Любая дист.</option>
                      <option value="self">На себя</option>
                      <option value="touch">Касание</option>
                      <option value="30">До 30 фт.</option>
                      <option value="60">До 60 фт.</option>
                      <option value="90">До 90 фт.</option>
                      <option value="120">До 120 фт.</option>
                    </select>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 items-center border-t border-amber-900/20 pt-2">
                    ...
                    <button onClick={() => setShowCategories(!showCategories)} className={`px-4 py-2 border rounded font-bold text-[10px] uppercase transition-colors ${showCategories ? 'bg-amber-500 text-black border-amber-400' : 'border-amber-800/40 text-amber-500'}`}>Категории</button>

                    <div className="flex gap-1 ml-auto">
                      <button onClick={() => { if(compareMode && compareList.length >= 2) setShowCompareResults(true); setCompareMode(!compareMode); }} className={`px-4 py-2 rounded font-black border-2 text-[10px] transition-all ${compareMode ? 'bg-amber-500 text-black border-white shadow-[0_0_15px_rgba(251,191,36,0.3)]' : 'bg-black text-amber-500 border-amber-900'}`}>
                  ...
                  {showCategories && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-[#efe7d6] text-black rounded-lg shadow-2xl z-[300] p-5 border-2 border-amber-700 grid grid-cols-2 md:grid-cols-4 gap-4 animate-in slide-in-from-top-2 duration-300">
                      <div onClick={() => {setCategoryFilter(null); setShowCategories(false);}} className="col-span-full cursor-pointer hover:bg-amber-200 p-2 font-black border-b-2 border-amber-900/10 mb-2 uppercase text-xs">Все доступные модули</div>
              ...
              <div className="flex flex-col lg:flex-row gap-8">
                <main className="flex-grow space-y-12">
                  ...
                  {Object.keys(filteredData).length === 0 && (
                    <div className="text-center py-20 opacity-20 italic uppercase font-black">Ни одного совпадения в архиве не найдено</div>
                  )}
                </main>
                ...
                <aside className="w-full lg:w-[320px] flex-shrink-0 sticky top-[160px] max-h-[calc(100vh-200px)] overflow-y-auto pr-2 custom-scrollbar font-sans">
                  <div className="bg-[#111] border-2 border-amber-600/30 rounded-xl p-3 space-y-2 shadow-2xl">
                    <h3 className="text-amber-500 font-black text-center text-[10px] uppercase mb-3 tracking-[0.2em] border-b border-amber-900/30 pb-2">★ Избранные модули ★</h3>
                    {favorites.length === 0 ? <p className="text-center text-amber-900 text-[10px] uppercase py-4">Архив пуст</p> :
                      favorites.map(f => (
        ...
        <div className="fixed inset-0 z-[500] bg-black/95 backdrop-blur-xl p-6 overflow-y-auto">
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-8 border-b border-amber-900/30 pb-4">
              <h2 className="text-3xl font-black text-amber-500 uppercase italic tracking-tighter">Сличительный анализ чертежей</h2>
              <button onClick={() => { setShowCompareResults(false); setCompareList([]); setCompareMode(false); }} className="px-8 py-3 bg-amber-600 text-black font-black rounded-lg hover:bg-amber-400 transition-colors shadow-lg">ВЫХОД</button>
            </div>
      ...
      {showCombatInfo && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md" onClick={() => setShowCombatInfo(false)}>
          <div className="bg-[#efe7d6] p-8 rounded-2xl max-w-sm w-full border-4 border-[#3d2314] text-center shadow-[0_0_50px_rgba(0,0,0,0.5)]">
            <h3 className="text-[#3d2314] font-black text-2xl uppercase mb-4 italic leading-tight border-b-2 border-[#3d2314]/20 pb-2">Инфо-панель</h3>
            <p className="text-[#4d2d1a] font-bold text-sm leading-relaxed">Система боя активирует упрощенный вывод избранных чертежей. Используйте ★ для пополнения боезапаса.</p>
            <button onClick={() => setShowCombatInfo(false)} className="mt-8 w-full bg-[#3d2314] text-amber-500 py-4 rounded-lg font-black uppercase shadow-xl hover:bg-[#2a180e] transition-all">Принято</button>
          </div>
        </div>
      )}
    </div>
